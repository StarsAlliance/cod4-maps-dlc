...
main()
{
	...
	drownage = getentarray("pronedrown","targetname");
...
//		IPrintLn( "Your Drowning!!!!" );
	...
	if(!isDefined(water_vision))
	{
		...
		water_vision.color = (.129412, .188235, .176471);
		water_vision.alpha = .95;
		
	}


	level.barincrement = (level.barsize / (10.0 * level.drowntime));
//    level.player allowProne(false);
	...
	if(!isDefined(self.progressbackground))
	{
		...
		self.progressbackground.y = 420;
		self.progressbackground.alpha = 0.5;
	}
	self.progressbackground setShader("black", (level.barsize + 4), 10);		
	...
	if(!isDefined(self.progressbar))
	{
		...
		self.progressbar.y = 420;
	}
	self.progressbar setShader("white", 0, 6);			
	self.progressbar scaleOverTime(level.drowntime, level.barsize, 6);
	...
	if(isalive(self) && self istouching(trigger) && !self istouching(dceiling) && (self.progresstime >= level.drowntime))
	{
		...
		randb = randomInt(4);
		deathmethod1 = " Tried to hold his breath too long!";	
		deathmethod2 = " paid a long lasting visit to Davy Jones's Locker.";
		deathmethod3 = " Forgot the scuba gear!";
	...
	{
		water_vision.alpha = .3;
		water_vision fadeOverTime(2);
